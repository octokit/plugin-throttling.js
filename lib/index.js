...
function throttlingPlugin (octokit, pluginOptions = {}) {
  const state = {
  ...
  octokit.throttle = Object.assign({
    _options: (options = {}) => Object.assign(state, options)
  }, pluginOptions.throttle)

  const events = {}
  const emitter = new Bottleneck.Events(events)
  events.on('abuse-limit', octokit.throttle.onAbuseLimit)
  events.on('rate-limit', octokit.throttle.onRateLimit)
