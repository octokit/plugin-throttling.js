...
function throttlingPlugin (octokit) {
  ...
  state.retryLimiter.on('failed', async function (error, info) {
    ...
    if (error.status === 403 && /\babuse\b/i.test(error.message)) {
      ...
      const wantRetry = await emitter.trigger('abuse-limit', retryAfter, retryCount, options)
      ...
      const wantRetry = await emitter.trigger('rate-limit', retryAfter, retryCount, options)
